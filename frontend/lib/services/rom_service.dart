...
class RomService {
  ...
  // Cache: Category_Key -> List<RomModel>
  final Map<String, List<RomModel>> _cache = {};

  // Default filters
  List<String> _regions = ["Europe", "France", "Fr", "USA", "Japan"];
  List<String> _exclude = ["Demo", "Beta", "Proto", "Kiosk", "Sample", "Unl"];

  // NOTE: Rename field to avoid conflict if needed, or just use boolean in method
  // But reusing the name "deduplicate" as argument vs field is fine.
  bool _defaultDeduplicate = true;
  ...
  void updateFilters({
    ...
    if (exclude != null) _exclude = exclude;
    if (deduplicate != null) _defaultDeduplicate = deduplicate;
  }

  /// Fetch file list from the configured URL for a console
  Future<List<RomModel>> fetchFileList(
    ...
    if (config == null) {
      throw Exception('Console config not found: $consoleKey');
    }
    ...
    try {
      ...
      print('✅ Found ${roms.length} files for $consoleKey');
      _cache[cacheKey] = roms;
      return roms;
    } catch (e) {
      print('❌ Error fetching file list: $e');
      return []; // Return empty list on error to not crash UI
    }
  }

  /// Extract file size from HTML element
  String _extractSize(Element link) {
    // Strategy 1: Table based (Myrient)
    ...
    // Strategy 2: Text based (Apache standard)
    // Manually find next text node
    if (link.parentNode != null) {
      final siblings = link.parentNode!.nodes;
      final index = siblings.indexOf(link);
      if (index != -1) {
        // Look ahead for text nodes
        for (var i = index + 1; i < siblings.length; i++) {
          ...
          if (node.nodeType == Node.TEXT_NODE &&
            ...
            break; // Stop after first text node found
          }
        }
      }
    }
  ...
  Future<List<RomModel>> search(
    ...
    for (var rom in roms) {
      ...
      // 2. Region Filter (if activeRegions provided)
      if (activeRegions.isNotEmpty) {
        final fileRegion = rom.region;
  ...
  Stream<double> downloadFile(
    ...
    // Handle Myrient/Archive encoding
